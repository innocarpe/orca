...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('ORCA_OMP_PREFILL_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('$env:ORCA_OMP_STATUS_EXTENSION')
    expect(script).toContain('$env:ORCA_OMP_PREFILL_EXTENSION')
