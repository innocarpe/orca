...
describe('ui-locale', () => {
  ...
  it('maps Traditional Chinese system locales to zh-TW', () => {
  ...
  it('maps system locale to the closest supported locale', () => {
    ...
    expect(resolveUiLocale(UI_LANGUAGE_SYSTEM, 'zh-HK')).toBe('zh-TW')
    expect(resolveUiLocale(UI_LANGUAGE_SYSTEM, 'zh-Hant')).toBe('zh-TW')
  ...
  it('uses renderer system locale only for the system setting', () => {
    ...
    expect(resolveRendererUiLocale(UI_LANGUAGE_CHINESE_TRADITIONAL)).toBe('zh-TW')
