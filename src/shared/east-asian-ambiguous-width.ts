...
/** True when Unicode East_Asian_Width(codepoint) is Ambiguous (A). */
export function isEastAsianAmbiguous(codepoint: number): boolean {
  ...
  while (lo <= hi) {
    const mid = (lo + hi) >> 1
    const range = AMBIGUOUS_RANGES[mid]!
...
export function resolveEastAsianAmbiguousCellWidth(
  codepoint: number,
  baseWidth: 0 | 1 | 2,
  mode: TerminalEastAsianAmbiguousWidth
): 0 | 1 | 2 {
  if (mode !== 'wide' || baseWidth !== 1) {
    return baseWidth
  }
  return isEastAsianAmbiguous(codepoint) ? 2 : 1
}
